#!/usr/bin/env python3
"""
MauStats MX — sidebar dark-pro dashboard
Inspired by bball-shot-explorer layout: fixed sidebar + routing + pitch visualization
"""
...
# PALETTE
# ─────────────────────────────────────────────────────────────────────────────
BG    = '#0b0f14'; BG2 = '#101620'; BG3 = '#0f151e'
CARD  = 'rgba(15,21,30,0.9)'; BORDER = '#1e2a3a'
WHITE = '#e6edf3'; MUTED = '#6e7681'; GRAY = '#8b949e'
RED   = '#D5001C'; GREEN = '#2ea043'; GOLD = '#FFD700'; DANGER = '#f85149'

TEAM_COLORS = {
    'Chivas':'#CD1F2D','Cruz Azul':'#1E6AE1','Toluca':'#D5001C',
    'América':'#FFD700','Tigres':'#F5A623','Monterrey':'#003DA5',
    'Pumas':'#C8A84B','Santos Laguna':'#2E8B57','Pachuca':'#A0B8C8',
    'Atlas':'#B22222','León':'#2D8C3C','Necaxa':'#D62828',
    'Tijuana':'#C62828','Querétaro':'#1A7FCB','FC Juárez':'#4CAF50',
    'Mazatlán':'#9B59B6','San Luis':'#D52B1E','Puebla':'#2563EB',
}
TEAM_IDS = {
...
def pimg(pid): return f'/images/players/{int(pid)}.png' if pid and pd.notna(pid) and (PLAYERS_IMG/f'{int(pid)}.png').exists() else None
def rgba(h,a):
...
# PLOTLY
# ─────────────────────────────────────────────────────────────────────────────
PBASE = dict(paper_bgcolor=BG, plot_bgcolor=BG2, font=dict(color=WHITE, family='Inter,Arial'),
             template='plotly_dark')
VERDE = [[0,BG],[0.04,'#1B5E20'],[0.15,'#2E7D32'],[0.40,'#00C853'],[1.0,'#00FF88']]

# ─────────────────────────────────────────────────────────────────────────────
# PLAYER POSITIONS → PITCH COORDS
# ─────────────────────────────────────────────────────────────────────────────
PITCH_XY = {
...
# ─────────────────────────────────────────────────────────────────────────────
RADAR_COLS = {
    'POR':['paradas_p90','porcentaje_paradas_p90','goles_evitados_p90','goles_recibidos_p90','porterias_cero_p90'],
    'DEF':['intercepciones_p90','entradas_p90','despejes_p90','duelos_tierra_ganados_p90','pases_precisos_p90'],
    'MED':['pases_precisos_p90','pases_largos_p90','recuperaciones_campo_rival_p90','chances_creadas_p90','asistencias_p90'],
    'DEL':['goles_p90','xG_p90','tiros_p90','tiros_a_puerta_p90','grandes_chances_p90'],
}
STAT_META = {
    'minutos_stats':('⏱','Minutos'),'partidos_stats':('📅','Partidos'),
    'goles':('⚽','Goles'),'asistencias':('🔑','Asistencias'),'rating':('⭐','Rating'),
    'goles_p90':('⚽','Goles/90'),'xG_p90':('📊','xG/90'),
    'tiros_p90':('🎯','Tiros/90'),'asistencias_p90':('🔑','Asis/90'),
    'pases_precisos_p90':('🎽','Pases/90'),'chances_creadas_p90':('💡','Chances/90'),
}
def pos_group(pos):
...
def load_model():
    ...
    for fp in sorted(glob.glob(str(HIST_DIR/'*.json'))):
        ...
        w=TORNEO_W.get(tkey)
        if not w: continue
        for p in json.load(open(fp,encoding='utf-8')).get('partidos',[]):
    ...
    for t in gf:
        ...
        att[t]=(gf[t]/m)/mu if m else 1.; defe[t]=(ga[t]/m)/mu if m else 1.
    return att,defe,mu,1.15

def predict(lo,vi,att,defe,mu,ha,mg=6):
    ll=att.get(lo,1.)*defe.get(vi,1.)*mu*ha; lv=att.get(vi,1.)*defe.get(lo,1.)*mu
    P=np.array([[poisson.pmf(i,ll)*poisson.pmf(j,lv) for j in range(mg+1)] for i in range(mg+1)])
    return float(np.sum(np.tril(P,-1))),float(np.trace(P)),float(np.sum(np.triu(P,1))),P,ll,lv

def montecarlo(att,defe,mu,ha,n=5000):
    d=json.load(open(HIST_DIR/'historico_clausura_2026.json',encoding='utf-8'))
    tb={norm(r['equipo']):{'pts':int(r.get('pts',0)),'gf':int(r.get('gf',0)),'gc':int(r.get('gc',0))} for r in d.get('tabla',[])}
    pend=[(norm(p['local']),norm(p['visitante'])) for p in d.get('partidos',[]) if not p.get('terminado')]
    ts=sorted(tb.keys()); N=len(ts); pc=np.zeros((N,N),dtype=np.int32)
    for _ in range(n):
        pts={t:tb[t]['pts'] for t in ts}; gf={t:tb[t]['gf'] for t in ts}; gc={t:tb[t]['gc'] for t in ts}
        for lo,vi in pend:
            if lo not in pts or vi not in pts: continue
            gl=np.random.poisson(att.get(lo,1.)*defe.get(vi,1.)*mu*ha)
...
# ─────────────────────────────────────────────────────────────────────────────
def load_elo():
    df=pd.read_csv(ELO_CSV,parse_dates=['fecha']); df['equipo']=df['equipo'].apply(norm); return df
def load_jug():
    df=pd.read_csv(JUG_CSV); df.columns=df.columns.str.strip('\ufeff')
    df['equipo']=df['equipo'].apply(norm); return df[df['minutos_stats']>=200].copy()
def load_hist():
    return json.load(open(HIST_DIR/'historico_clausura_2026.json',encoding='utf-8'))
def load_j13(hist):
    return [(norm(p['local']),norm(p['visitante'])) for p in hist.get('partidos',[]) if p.get('jornada')=='13']

print("Loading Poisson model…"); ATT,DEFE,MU,HA = load_model()
print("Monte Carlo (5k)…");     MC_T,MC_P = montecarlo(ATT,DEFE,MU,HA)
print("ELO + players…");        ELO=load_elo(); JUG=load_jug()
...
# ─────────────────────────────────────────────────────────────────────────────
def empty_fig(h=400):
    return go.Figure(layout=dict(paper_bgcolor=BG,plot_bgcolor=BG2,height=h,
                                  margin=dict(l=0,r=0,t=0,b=0),
                                  xaxis=dict(visible=False),yaxis=dict(visible=False)))

def fig_heatmap(probs,lo,vi):
    mg=probs.shape[0]-1; z=(probs*100).round(1)
    txt=[[f'{z[i,j]:.1f}%' for j in range(mg+1)] for i in range(mg+1)]
    fig=go.Figure(go.Heatmap(z=z,x=list(range(mg+1)),y=list(range(mg+1)),
        text=txt,texttemplate='%{text}',textfont=dict(size=9),
        colorscale=VERDE,showscale=True,zmin=0,zmax=30,
        colorbar=dict(tickfont=dict(color=MUTED,size=9),title=dict(text='%',font=dict(color=MUTED))),
...
def fig_elo_rank():
    lat=ELO.sort_values('fecha').groupby('equipo').last().reset_index().sort_values('elo',ascending=True)
    cols=[TEAM_COLORS.get(t,RED) for t in lat['equipo']]
    imgs=[dict(source=f'/images/teams/{TEAM_IDS[t]}.png',xref='paper',yref='y',
               x=-0.01,y=t,sizex=0.04,sizey=0.75,xanchor='right',yanchor='middle',layer='above')
          for t in lat['equipo'] if t in TEAM_IDS and (TEAMS_IMG/f'{TEAM_IDS[t]}.png').exists()]
    fig=go.Figure(go.Bar(x=lat['elo'],y=lat['equipo'],orientation='h',
        marker_color=cols,marker_line_width=0,
        text=lat['elo'].round(0).astype(int),textposition='outside',
...
def fig_elo_hist(teams):
    ...
    for t in (teams or []):
        ...
        if d.empty: continue
        fig.add_trace(go.Scatter(x=d['fecha'],y=d['elo'],mode='lines',name=t,
    ...
    fig.update_layout(**PBASE,
        ...
        yaxis=dict(tickfont=dict(color=MUTED),gridcolor='#1e2530',title='ELO',range=[1300,None]),
...
def fig_mc():
    ...
    z=(pso*100).round(1)
    txt=[[f'{z[i,j]:.0f}%' if z[i,j]>=3 else '' for j in range(n)] for i in range(n)]
    fig=go.Figure(go.Heatmap(z=z,x=[str(i+1) for i in range(n)],y=tso,
        text=txt,texttemplate='%{text}',textfont=dict(size=10,color=WHITE),
        colorscale=VERDE,showscale=True,zmin=0,zmax=70,
        colorbar=dict(title=dict(text='%',font=dict(color=MUTED)),tickfont=dict(color=MUTED)),
        hovertemplate='<b>%{y}</b><br>Pos %{x}: %{z:.1f}%<extra></extra>'))
    fig.add_shape(type='rect',x0=-0.5,x1=3.5,y0=-0.5,y1=n-0.5,fillcolor='rgba(255,215,0,0.05)',line=dict(width=0),layer='below')
    fig.add_shape(type='rect',x0=3.5,x1=7.5,y0=-0.5,y1=n-0.5,fillcolor='rgba(46,160,67,0.05)',line=dict(width=0),layer='below')
    fig.add_shape(type='line',x0=3.5,x1=3.5,y0=-0.5,y1=n-0.5,line=dict(color=GOLD,width=2,dash='dot'))
    fig.add_shape(type='line',x0=7.5,x1=7.5,y0=-0.5,y1=n-0.5,line=dict(color=GREEN,width=2,dash='dot'))
    # Bold overlay for ≥20%
    bx,by,bt=[],[],[]
    for i,t in enumerate(tso):
        for j in range(n):
            if z[i,j]>=20: bx.append(str(j+1)); by.append(t); bt.append(f'<b>{z[i,j]:.0f}%</b>')
    if bx: fig.add_trace(go.Scatter(x=bx,y=by,mode='text',text=bt,
            textfont=dict(size=14,color=WHITE,family='Arial Black'),hoverinfo='skip',showlegend=False))
    fig.add_annotation(x=1.5,y=-0.06,xref='x',yref='paper',text='◆ LIGUILLA',
        showarrow=False,font=dict(color=GOLD,size=10,family='Arial Black'))
    fig.add_annotation(x=5.5,y=-0.06,xref='x',yref='paper',text='◆ REPECHAJE',
        showarrow=False,font=dict(color=GREEN,size=10,family='Arial Black'))
    fig.update_layout(**PBASE,
        title=dict(text='Simulación Monte Carlo — Clausura 2026',font=dict(size=15,color=WHITE),x=0.5,y=0.98),
        xaxis=dict(title='',tickfont=dict(color=WHITE,size=11),side='top',gridcolor='#1e2530'),
        yaxis=dict(tickfont=dict(size=12,color=WHITE),autorange='reversed',gridcolor='#1e2530'),
        height=710,margin=dict(l=150,r=40,t=65,b=55))
    return fig

def fig_pitch(team=None, highlight_player=None):
    """Football pitch with players as scatter dots."""
    # ── Pitch shapes ──
    ...
    shapes=[
        ...
        # Stripe effect (alternating bands)
    ...
    if team:
        ...
        # Rating color scale: red→yellow→green
        def rating_color(r):
        ...
        for _,p in players.iterrows():
            ...
            mins=p.get('minutos_stats',0) or 0
            sizes.append(8+min(mins/90*0.7,10))
            g=round(float(p.get('goles',0) or 0),0)
            a=round(float(p.get('asistencias',0) or 0),0)
            rat=round(float(r),2) if pd.notna(r) else '—'
            pos=str(p.get('posicion','')).split(',')[0]
            hovers.append(f"<b>{p['nombre']}</b><br>{pos}<br>⭐ {rat} · ⚽ {int(g)} · 🔑 {int(a)}<br>⏱ {int(mins)} min")
        ...
        # All players (non-highlighted)
        nl_xs=[x for x,h in zip(xs,is_hl) if not h]
        nl_ys=[y for y,h in zip(ys,is_hl) if not h]
        nl_colors=[c for c,h in zip(colors,is_hl) if not h]
        nl_sizes=[s for s,h in zip(sizes,is_hl) if not h]
        nl_hovers=[hv for hv,h in zip(hovers,is_hl) if not h]

        fig.add_trace(go.Scatter(x=nl_xs,y=nl_ys,mode='markers',
            marker=dict(color=nl_colors,size=nl_sizes,
                        line=dict(color='rgba(0,0,0,0.5)',width=1),opacity=0.9),
            text=nl_hovers,hovertemplate='%{text}<extra></extra>',
            showlegend=False))

        # Highlighted player
        if hl_xs:
            fig.add_trace(go.Scatter(x=hl_xs,y=hl_ys,mode='markers+text',
...
def fig_radar(row):
    ...
    if not cols: return empty_fig(380)
    same=JUG[JUG['posicion'].apply(pos_group)==pg]
    vals,lbls=[],[]
    for c in cols:
        v=float(row.get(c,0) or 0); cd=same[c].dropna()
    ...
    fig=go.Figure()
    for v in [25,50,75]:
        fig.add_trace(go.Scatterpolar(r=[v]*len(lc),theta=lc,mode='lines',
            line=dict(color='#2d333b',width=0.7,dash='dot'),showlegend=False,hoverinfo='skip'))
    fig.add_trace(go.Scatterpolar(r=vc,theta=lc,fill='toself',fillcolor=rgba(tc,0.45),
    ...
    fig.update_layout(
        ...
        polar=dict(bgcolor=BG2,
            ...
            angularaxis=dict(tickfont=dict(color=WHITE,size=10),gridcolor='#2d333b',linecolor='#2d333b')),
...
def fig_comp(r1,r2):
    pg1=pos_group(r1.get('posicion','DEL')); pg2=pos_group(r2.get('posicion','DEL'))
    cols=[c for c in RADAR_COLS.get(pg1,RADAR_COLS['DEL'])
          if c in JUG.columns and c in RADAR_COLS.get(pg2,RADAR_COLS['DEL'])]
    if not cols: cols=[c for c in RADAR_COLS['DEL'] if c in JUG.columns]
    if not cols: return empty_fig(460)
    pool=JUG[JUG['posicion'].apply(pos_group).isin([pg1,pg2])]
    lbls,p1,p2,v1,v2=[],[],[],[],[]
    for c in cols:
        cd=pool[c].dropna()
        vv1=float(r1.get(c,0) or 0); vv2=float(r2.get(c,0) or 0)
        ...
        lbls.append(c.replace('_p90','').replace('_',' ').upper())
    c1=TEAM_COLORS.get(r1.get('equipo',''),RED); c2=TEAM_COLORS.get(r2.get('equipo',''),'#4CAF50')
    n1=r1.get('nombre','J1'); n2=r2.get('nombre','J2')
    fig=go.Figure()
    fig.add_trace(go.Bar(y=lbls,x=[-x for x in p1],orientation='h',name=n1,
        marker_color=c1,marker_line_width=0,
        text=[f'{x:.2f}' for x in v1],textposition='inside',textfont=dict(color=WHITE,size=10),
        customdata=v1,hovertemplate=f'<b>{n1}</b><br>%{{y}}: %{{customdata:.2f}}<extra></extra>'))
    fig.add_trace(go.Bar(y=lbls,x=p2,orientation='h',name=n2,
        marker_color=c2,marker_line_width=0,
        text=[f'{x:.2f}' for x in v2],textposition='inside',textfont=dict(color=WHITE,size=10),
        customdata=v2,hovertemplate=f'<b>{n2}</b><br>%{{y}}: %{{customdata:.2f}}<extra></extra>'))
    fig.update_layout(**PBASE,barmode='overlay',
        title=dict(text=f'<b>{n1}</b>  vs  <b>{n2}</b> — por percentil',font=dict(size=13,color=WHITE),x=0.5),
...
def build_standings():
    ...
    for r in HIST.get('tabla',[]):
        pos=int(r.get('pos',0)); t=norm(r['equipo']); tid=TEAM_IDS.get(t); tc=TEAM_COLORS.get(t,GRAY)
        ...
        rows.append(html.Tr([
            ...
            html.Td(html.Div([
                html.Img(src=f'/images/teams/{tid}.png',height='18px',
                         style={'marginRight':'6px','verticalAlign':'middle','filter':'drop-shadow(0 1px 3px rgba(0,0,0,0.6))'}) if tid else None,
                html.Span(t,style={'color':tc,'fontWeight':'600'})],className='team-cell')),
            ...
            html.Td(f'+{dg}' if dg>0 else str(dg),
                style={'color':GREEN if dg>0 else (DANGER if dg<0 else MUTED),'textAlign':'center','fontWeight':'600'}),
        ],className=_zone(pos),style={'backgroundColor':'rgba(15,21,30,0.8)' if pos%2==0 else 'rgba(22,27,34,0.5)'}))
    return html.Div([
        html.Div(className='section-ttl',children='Tabla de posiciones'),
        html.Div([
...
def build_results():
    ...
    return html.Div([html.Div('Últimos resultados — J12',className='section-ttl'),html.Div(cards)])
...
def build_upcoming():
    ...
    return html.Div([html.Div('Próxima jornada — J13',className='section-ttl'),html.Div(cards)])
...
def make_sidebar(pathname='/'):
    ...
    elo_rows=[html.Div([
        ...
        html.Img(src=f'/images/teams/{TEAM_IDS.get(r["equipo"],0)}.png',
                 height='16px') if r['equipo'] in TEAM_IDS else None,
        html.Span(r['equipo'],style={'color':TEAM_COLORS.get(r['equipo'],GRAY),'fontSize':'10px','fontWeight':'600'}),
    ...
    nav_items=[
        html.A([
            ...
            html.Span(label),
        ],href=href,className='nav-item'+(' active' if pathname==href or (pathname=='/' and href=='/') else ''))
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.Img(src='/images/teams/7807.png',height='20px') if (TEAMS_IMG/'7807.png').exists() else None,
                html.Div([
            ...
            html.Div('ELO TOP 3',style={'fontSize':'9px','letterSpacing':'2px','color':'#30363d','fontWeight':'700','marginBottom':'6px','textTransform':'uppercase'}),
...
def page_jornada():
    ...
    for lo,vi in J13:
        tid_lo=TEAM_IDS.get(lo); tid_vi=TEAM_IDS.get(vi)
        lbl=html.Span([
            html.Img(src=f'/images/teams/{tid_lo}.png',height='18px',style={'marginRight':'5px','verticalAlign':'middle'}) if tid_lo else None,
            html.Span(lo,style={'color':TEAM_COLORS.get(lo,WHITE),'fontWeight':'600'}),
            html.Span(' vs ',style={'color':MUTED,'margin':'0 5px'}),
            html.Img(src=f'/images/teams/{tid_vi}.png',height='18px',style={'marginRight':'5px','verticalAlign':'middle'}) if tid_vi else None,
    ...
    return html.Div([
        ...
        html.Div([
            dbc.Row(dbc.Col(dcc.Dropdown(id='dd-match',options=opts,
                value=opts[0]['value'] if opts else None,clearable=False,optionHeight=38,
            ...
            dbc.Row([
                ...
                dbc.Col(html.Div([
                    dcc.Graph(id='graph-heatmap',figure=empty_fig(340),config={'displayModeBar':False}),
...
def page_elo():
    ...
    return html.Div([
        ...
        html.Div([
            dbc.Row([
                dbc.Col(html.Div(dcc.Graph(figure=FIG_ELO_RANK,config={'displayModeBar':False}),className='g-card'),md=5),
                dbc.Col([
                    html.Div([
                        html.Div('Comparar equipos',className='g-card-title',style={'padding':'12px 16px 0'}),
                        html.Div(dcc.Dropdown(id='dd-elo-teams',
                    ...
                    html.Div(dcc.Graph(id='graph-elo-hist',config={'displayModeBar':False}),className='g-card'),
                ],md=7),
            ],className='g-3'),
        ],className='page-content'),
    ])
...
def page_sim():
    return html.Div([
        ...
        html.Div([
            html.Div(dcc.Graph(figure=FIG_MC,config={'displayModeBar':False}),className='g-card gap-col'),
            html.Div([
    ...
    return html.Div([
        ...
        html.Div([
            dbc.Row([
                ...
                # Right panel: controls + profile + stats
                dbc.Col([
                    html.Div([
                        html.Div('EQUIPO',className='section-ttl'),
                        dcc.Dropdown(id='dd-team-jug',options=[{'label':t,'value':t} for t in teams],
        ...
        html.Div([
            dbc.Row([
                dbc.Col([
                    ...
                    dcc.Dropdown(id='comp-team-1',options=[{'label':t,'value':t} for t in teams],
                        value=teams[0],clearable=False,
                        style={'backgroundColor':BG3,'border':f'1px solid {BORDER}','borderRadius':'7px','marginBottom':'8px'}),
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(id='comp-team-2',options=[{'label':t,'value':t} for t in teams],
            ...
            html.Div(dcc.Graph(id='graph-comp',figure=empty_fig(460),config={'displayModeBar':False}),
                     className='g-card'),
...
def cb_j13(val):
    ...
    def circ(pct,lbl,color,team,src):
        return html.Div([
            ...
            ],className='prob-circle',style={'borderColor':color,'boxShadow':f'0 0 20px {rgba(color,0.2)} inset','backgroundColor':rgba(color,0.06)}),
...
# ─────────────────────────────────────────────────────────────────────────────
@app.callback(
    Output('dd-jug','options'), Output('dd-jug','value'),
...
def cb_player(player,team):
    if not team: return fig_pitch(),empty_fig(380),html.Div()
    pitch=fig_pitch(team,player)
    if not player: return pitch,empty_fig(380),html.Div(html.Div('Selecciona un jugador',style={'color':MUTED,'textAlign':'center','padding':'20px'}))
    rows=JUG[(JUG['equipo']==team)&(JUG['nombre']==player)]
    if rows.empty: return pitch,empty_fig(380),html.Div()
    row=rows.iloc[0]
    ...
    profile=html.Div([
        html.Div([
            ...
            html.Img(src=f'/images/teams/{tid}.png',height='24px',
                     style={'position':'absolute','bottom':'0','right':'0','filter':'drop-shadow(0 1px 3px rgba(0,0,0,0.9))'}) if tid else None,
    ...
    for f in key_fields:
        ...
        vf=float(val)
        disp=(f'{vf:.2f}' if '_p90' in f else f'{vf:.1f}' if f=='rating'
              else f'{int(vf):,}' if f=='minutos_stats' else str(int(vf)) if vf==int(vf) else f'{vf:.2f}')
        pills.append(html.Div([
            html.Div(meta[0],className='stat-icon'),
            html.Div([html.Div(disp,className='stat-v',style={'color':tc}),html.Div(meta[1],className='stat-l')],className='stat-txt'),
        ],className='stat-pill'))
...
# ─────────────────────────────────────────────────────────────────────────────
@app.callback(
    Output('comp-p1','options'), Output('comp-p1','value'), Input('comp-team-1','value'))
def cb_ct1(t):
    if not t: return [],None
    ps=JUG[JUG['equipo']==t]['nombre'].dropna().sort_values().tolist()
    o=[{'label':p,'value':p} for p in ps]; return o,(o[0]['value'] if o else None)

@app.callback(
    Output('comp-p2','options'), Output('comp-p2','value'), Input('comp-team-2','value'))
def cb_ct2(t):
    if not t: return [],None
    ps=JUG[JUG['equipo']==t]['nombre'].dropna().sort_values().tolist()
    o=[{'label':p,'value':p} for p in ps]; return o,(o[0]['value'] if o else None)
